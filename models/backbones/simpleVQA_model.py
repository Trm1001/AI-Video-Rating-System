...
class ResNet(nn.Module):
    ...
    def forward(self, batch, multi=None, layer=None):
        # See note [TorchScript super()]
        # input dimension: batch x frames x 3 x height x width
        x = batch['simpleVQA']
        b,c,T,h1,w1 = x.shape
        x = x.permute(0,2,1,3,4).contiguous()
        x_3D_features =  batch['feat'].to(batch['simpleVQA'].device)
        x_size = x.shape
...
def resnet50(pretrained=True, progress=True, **kwargs):
    ...
    if pretrained:
        ...
        pre_train_model = {k:v for k,v in pre_train_model.items() if k in model_dict}
        model_dict.update(pre_train_model)
        print('load from pretraimed resnet:',model.load_state_dict(model_dict))
    return model
